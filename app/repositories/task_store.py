...
线程安全：单例 + 单连接 + threading.Lock 保护所有写操作。
SQLite 事务是连接级别的，多线程共用一个连接时必须串行化写操作，
否则会出现 "cannot commit - no transaction is active" 等事务交叉冲突。
读操作不需要锁（SQLite 支持并发读）。
"""
...
class TaskStore:
    ...
    所有写操作都在 self._lock 保护下执行，确保多线程环境下事务不交叉。
    """
